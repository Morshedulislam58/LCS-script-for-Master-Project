...
csv_path = r"E:\LCS_Project_File\Signal_Reconstruction\scope_32_100.csv"
# ---------------- Load CSV ----------------
times, volts, digital = [], [], []
...
# ---------------- Event positions digital ----------------
crossing_point = np.where(np.diff(event_code) != 0)[0] + 1
event = np.unique(np.r_[0, crossing_point, len(times) - 1])
event_times = times[event]
event_values = x[event]
...
# ---------------- Average sample rate  ----------------
avg_sample_rate = len(event_times) / ((times[-1] - times[0]) / 1000.0)  
